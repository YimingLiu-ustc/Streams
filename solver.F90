...
!
  icyc   = ncyc0  ! Reset the cycle counter
  telaps = telaps0 ! Reset the elapsed time
!
...
!
  ...
  dtmin = abs(cfl)
  if (cfl>0._mykind) call step() ! Compute the time step
...
!
    if (mod(i,nprint)==0) then ! 残差计算与输出
      call computeresidual() !计算当前的残差，用于评估解的收敛性。
      call printres() !输出当前的残差。
    endif
!

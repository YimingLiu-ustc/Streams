...
!
  sqgmr2  = sqgmr*(1._mykind+s2tinf) ! 计算粘度系数
  sqgmr2h = sqgmr2*0.5_mykind        ! 预计算常数
!
...
!
        ...
        fhat_gpu(i,j,k,6) = div3l !divergence of velocity
        ...
        rmuty   = drmutdt*ty
        rmutz   = drmutdt*tz
        sig11 = 2._mykind*(ux-div3l)  !粘性应力张量
        sig12 = uy+vx
        sig13 = uz+wx
        sig22 = 2._mykind*(vy-div3l)
        sig23 = vz+wy
        sig33 = 2._mykind*(wz-div3l)
        sigx  = rmutx*sig11       +  rmuty*sig12       +  rmutz*sig13       +  rmut*(ulap) ! 粘性力计算 F_x = ∂σ_xx/∂x + ∂σ_xy/∂y + ∂σ_xz/∂z tau11
        sigy  = rmutx*sig12       +  rmuty*sig22       +  rmutz*sig23       +  rmut*(vlap) ! tau21
        sigz  = rmutx*sig13       +  rmuty*sig23       +  rmutz*sig33       +  rmut*(wlap) ! tau31
        sigqt = (rmutx*tx+rmuty*ty+rmutz*tz+rmut*tlap)*ggmopr ! Conduction ! 热通量计算 q_x = -μ/Pr ∇T
...
!
        sigq = sigx*uu+sigy*vv+sigz*ww+sigah+sigqt ! b_x
!

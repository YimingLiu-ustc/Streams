...
!
  mydev=0
  call mpi_comm_split_type(mpi_comm_world,mpi_comm_type_shared,0,mpi_info_null,local_comm,iermpi) ! 按节点拆分通信域
  call mpi_comm_rank(local_comm,mydev,iermpi) ! 获取节点内rank
  iermpi = cudaSetDevice(mydev) ! 为每个MPI进程分配不同的GPU设备
...
!
  nblocks(2) = 1 ! 1D decomposition in y direction
!
  if(nblocks(1) <=0 .or. nblocks(3) <=0) then !如果输入文件中没有指定进程分布，则自动计算最优的 x 和 z 方向进程分布。
    dims = [0,0]
...
!
  ...
  if (iflow==3) then ! Rayleigh-Taylor instability (periodicity also applied in x)
    pbc(2) = .true.
  endif
!
! Create 3D topology

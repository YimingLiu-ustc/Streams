...
1003 format(I3.3)
!
  if (visc_type==2) s2tinf = 110.4_mykind/tref_dimensional ! Sutherland常数
!
  rho0 = 1._mykind
  u0   = sqrt(gamma)*rm
  v0   = 0._mykind
...
!
  ...
  elseif (iflow==3) then
    reout=retauinflow   !文件输入雷诺数
  endif
!
  re = reout
  sqgmr = sqrt(gamma)*rm/re
  if (iflow==3) sqgmr=1._mykind/re
! Adiabatic wall temperature and effective wall temperature
!
  taw   = t0*(1._mykind+0.5_mykind*gm1*rm*rm*rfac) ! 绝热壁温
  twall = taw*trat    ! 实际壁温（trat=0为温度比）
!
  if (iflow==-1) twall = t0
  if (iflow==0)  twall = t0
!
...
!
  ...
  winf(3) = rho0*v0
  winf(4) = rho0*w0
  winf(5) = p0*gm+0.5_mykind*(winf(2)**2)/winf(1) !Total Energy Density
!
...
!
  if (iflow>0 .and. iflow<3) call df_par() ! DF parameters (not needed for channel flow)
!

...
!integer, parameter :: quadtype = selected_real_kind(33,4931) ! extended precision
  ...
  integer, parameter :: mpi_prec = mpi_real8 !对应的 MPI 数据类型，用于并行计算时的数据通信
  real(mykind), parameter :: tol_iter = 0.000000001_mykind !迭代计算的容差，用于控制数值计算的精度要求
#endif
!
  integer, parameter :: nv      =  6   ! Physical variables
...
!
  ...
  real(mykind), parameter :: Fr     = 100._mykind/9 ! Fr=1/Ma**2 再所有case里是定值吗？如果不是，需要修改readinp从文件读入。其它参数同
  real(mykind), parameter :: At     = 0.3_mykind
...
!real(mykind), parameter :: pr     = 0.455_mykind
  ...
  real(mykind) :: vtexp  = 3._mykind/4._mykind
  ...
  real(mykind) :: rho0,t0,p0,u0,v0,w0,s0,t1
...
!
  ...
  character(6) :: stat_io
!
! rho1bc and rho2bc boundary data
  real(mykind), dimension(:,:), allocatable :: rho1bc, rho2bc, T1bc, T2bc
  real(mykind), dimension(:,:), allocatable :: rho1bc_gpu, rho2bc_gpu, T1bc_gpu, T2bc_gpu
...
!
  ...
  attributes(device) :: rho1bc_gpu, rho2bc_gpu
  attributes(device) :: T1bc_gpu, T2bc_gpu
!

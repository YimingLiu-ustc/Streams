...
#   FC = mpipgifort
    FC = mpif90
    ifeq (${MODE},"opt") 
        OLEVEL = -O3 -Mcuda=ccall,cuda11.3 -DUSE_CUDA -DCUDA_ASYNC -DNOCUDAAWAREMPI
    else
        OLEVEL = -Mcuda=ccall,cuda11.3 -DUSE_CUDA -DCUDA_ASYNC
    endif
endif
...
ifeq (${COMPILE},"pgi-cuda-sync") # mpif90 shipped with PGI compiler
    FC = mpif90 
    ifeq (${MODE},"opt") 
        OLEVEL = -O3 -Mcuda=ccall,cuda11.3 -DUSE_CUDA
    else
        OLEVEL = -Mcuda=ccall,cuda11.3 -DUSE_CUDA
    endif
endif
...
#----------------------------------------------------------------------
MAIN = main.o
OBJ_FILES = bc_non_reflecting.o init_rt.o bcwall_adiabatic.o \
    alloc.o bcdf.o bcextr.o bcfree.o bc.o bcrecyc.o bcrelax.o bcshk.o bcswap.o \
...
OBJ_MODS = mod_streams.o mod_sys.o euler.o Gauss_Err_Func.o
